...
Implement an API-first backend in the service module, seed a large dataset, expose match result endpoints for listing/searching/adding/removing records, and add a simple browser-based UI under the app folder that consumes the API. Keep the implementation modular and maintainable by separating domain, persistence, service, and presentation concerns.
